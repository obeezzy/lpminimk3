...
def find_launchpads():
    ...
    for system_port_name in found_launchpad_out_ports:
        client_name, port_name = _SystemMidiPortParser.extract_names(system_port_name)
        client_number, port_number = _SystemMidiPortParser.extract_numbers(system_port_name)
        midi_client = _MidiClient(client_name, client_number)
        port_index = _out_ports.index(system_port_name)
        midi_port = _MidiPort(port_name, port_number, port_index, system_port_name, direction=_MidiPort.OUT, midi_out=_midi_out)
    ...
    for system_port_name in found_launchpad_in_ports:
        ...
        midi_client = _MidiClient(client_name, client_number)
        midi_port = _MidiPort(port_name, port_number, port_index, system_port_name, midi_in=_midi_in, direction=_MidiPort.IN)
